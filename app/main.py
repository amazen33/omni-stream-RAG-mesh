...
def ingest(req: IngestRequest, x_request_id: str | None = Header(default=None)) -> dict[str, Any]:
    ...
    safe, counts = redact(req.text, os.getenv("PII_TOKEN_SALT", "change-me"))
    chunks = _chunks(safe)
    store.add([f"{rid}-{i}" for i in range(len(chunks))], chunks)
    ...
    payload = {"event": "ingest", "source": req.source, "pii_counts": counts,
               "chunk_count": len(chunks), "chunks": [{"index": i, "text": c} for i, c in enumerate(chunks)]}
    try:
...
def ask(req: AskRequest, x_request_id: str | None = Header(default=None)) -> dict[str, Any]:
    rid = request_id(x_request_id)
    safe_question, _ = redact(req.question, os.getenv("PII_TOKEN_SALT", "change-me"))
    retrieved = store.query(safe_question, req.top_k)
    prompt = f"Answer using retrieved evidence only.\nQuestion: {safe_question}\nEvidence: {retrieved}"
    output = os.getenv("RAG_FALLBACK_MESSAGE", "No indexed evidence is available.")
    if retrieved and os.getenv("ENABLE_OLLAMA_INFERENCE", "false").lower() == "true":
