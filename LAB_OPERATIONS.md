...
## Bootstrap
Copy `.env.example` to `.env`, use a password manager, create MinIO bucket with object locking at creation, then `docker compose up -d`. In Kubernetes, install cert-manager, Strimzi, Flink operator, ArgoCD and Argo Rollouts; apply `k8s/rag.yaml`, Kafka and streaming manifests. Configure External Secrets to cloud KMS/Key Vault before enabling sync.
