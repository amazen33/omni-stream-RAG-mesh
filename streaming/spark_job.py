"""Spark Structured Streaming job for domain events.

Run with spark-submit after installing the Kafka and Parquet connectors.
"""
...
spark = SparkSession.builder.appName("rag-domain-events").getOrCreate()
events = spark.readStream.format("kafka") \
    .option("kafka.bootstrap.servers", "kafka:9092") \
    .option("subscribe", "telemetry.ingested,transaction.processed,audit.record.logged") \
...
query = aggregated.writeStream.format("parquet") \
    .option("path", "s3a://streaming/events") \
    .option("checkpointLocation", "s3a://streaming/checkpoints/domain-events") \
